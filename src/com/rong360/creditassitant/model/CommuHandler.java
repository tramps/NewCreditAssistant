...
public class CommuHandler {
	...
	public static ArrayList<Communication> getAllSmsByTel(Context context,
		...
		Cursor c = context.getContentResolver().query(smsUri, smsProjection,
	...
	public static ArrayList<Communication> getAllCallLog(Context context) {
		...
		Cursor c = context.getContentResolver().query(
				CallLog.Calls.CONTENT_URI, callLogProjection, null, null,
				CallLog.Calls.DEFAULT_SORT_ORDER);

		HashMap<String, Customer> phone2NameMap = GlobalValue.getIns()
				.getPhoneNameMap();
		ArrayList<String> contacts = GlobalValue.getIns().getContactPhones(
				context);
	...
	public static ArrayList<Communication> getCallLogByTel(Context context,
		...
		String pureTem = TelHelper.getPureTel(tel);
		Cursor c = context.getContentResolver().query(
				CallLog.Calls.CONTENT_URI, callLogProjection,
				"number like \'%" + pureTem + "\'", null,
				CallLog.Calls.DEFAULT_SORT_ORDER);
		Communication com;
		while (c.moveToNext()) {
			com = buildCallLog(c);
