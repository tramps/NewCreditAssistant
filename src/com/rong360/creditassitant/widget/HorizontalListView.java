...
public class HorizontalListView extends AdapterView<ListAdapter> {
    ...
    public void setMovingBar(MovingBarView bar) {
	mMovingBar = bar;
    }
	...
	child.measure(
		MeasureSpec.makeMeasureSpec(
			...
			MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(120,
			MeasureSpec.AT_MOST));
    }
...
//	removeNonVisibleItems(dx);
	...
	if (mMovingBar != null) {
	    View v = getChildAt(0);
	    mMovingBar.initRect(v.getWidth(), v.getHeight(), top);
	}
    }
	...
	while (rightEdge + dx < getWidth()
	    ...
	    View child =
		    mAdapter.getView(mRightViewIndex, mRemovedViewQueue.poll(),
			    this);
	...
	while (leftEdge + dx > 0 && mLeftViewIndex >= 0) {
	    View child =
		    mAdapter.getView(mLeftViewIndex, mRemovedViewQueue.poll(),
			    this);
    ...
    public synchronized void setCurrentX(int x) {
	mNextX = x;
	requestLayout();
    }
    ...
    private OnGestureListener mOnGesture =
		...
		private boolean isEventWithinView(MotionEvent e, View child) {
		    ...
		    return viewRect.contains((int) e.getRawX(),
			    (int) e.getRawY());
		}
	    };
